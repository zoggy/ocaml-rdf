...
static void ml_final_##type (value val) \
{ refcount_pointer* p = (refcount_pointer*) Field(val,1); \
  if (p) { \
    * (p->cpt) = * (p->cpt) - 1; \
    if (* (p->cpt) == 0) final ((type*)p->data); \
  free(p);\
  } } \
static struct custom_operations ml_custom_##type = \
{ #type"/2.0/", ml_final_##type, custom_compare_default, \
...
CAMLprim value val_##type (type *p) \
{ value ret; if (!p) ml_raise_null_pointer(); \
  refcount_pointer* s = (refcount_pointer*) malloc(sizeof(refcount_pointer)); \
  s->cpt = 1; \
  s->data = (void*)p; \
  ret = alloc_custom (&ml_custom_##type, sizeof(value), adv, 1000); \
  initialize (&Field(ret,1), (value) s); init(p); return ret; \
} \
CAMLprim type * type##_val (value val) \
{ \
  refcount_pointer* p = (refcount_pointer*) Field(val,1); \
  return((type *)(p->data)); \
} \
CAMLprim value val_##type##_copy (value val) \
{ \
 value ret; \
 refcount_pointer* p = (refcount_pointer*) Field(val,1); \
